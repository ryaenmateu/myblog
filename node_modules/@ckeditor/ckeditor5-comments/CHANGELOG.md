...
* Comments feature now uses recently introduced marker-to-data conversion. This means that comment markers included in the document data returned from the editor will be different than earlier. The change is backward compatible in regards to old documents, that you might already have saved in your database. Those documents will be correctly processed by the editor, however, upon saving, the document data will be returned in the new format.

### Bug fixes

* Passing `Rect` or a callback returning `Rect` as an annotation target should work correctly now.
